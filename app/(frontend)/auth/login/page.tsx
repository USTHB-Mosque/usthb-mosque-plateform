...
export default function LoginPage() {
  ...
  return (
    <div className="flex flex-col lg:flex-row w-full min-h-screen">
      {/* Image Section - Left */}
      <div className="w-full lg:w-1/2 p-3 sm:p-4 order-1 lg:order-1">
      ...
      {/* Form Section - Right */}
      <div className="w-full lg:w-1/2 flex flex-col justify-center px-5 sm:px-8 md:px-12 lg:px-[140px] py-6 sm:py-8 lg:py-[48px] order-2 lg:order-2">
        {/* Logo */}
        ...
        <div className="text-center mb-5 sm:mb-6 lg:mb-8">
          <h1 className="text-xl sm:text-2xl md:text-3xl lg:text-[32px] font-bold text-gray-900 mb-1 sm:mb-2">أهلاً بعودتك</h1>
          <p className="text-xs sm:text-sm md:text-base lg:text-base text-gray-500">سجل دخولك إلى حسابك</p>
        </div>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3 sm:space-y-4 lg:space-y-5">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-gray-700 text-xs sm:text-sm md:text-base">البريد الإلكتروني</FormLabel>
                  <FormControl>
                    <Input placeholder="example@mail.com" {...field} disabled={isPending} className="h-10 sm:h-11 lg:h-12 text-xs sm:text-sm" />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-gray-700 text-xs sm:text-sm md:text-base">كلمة المرور</FormLabel>
                  <FormControl>
                    <Input type="password" placeholder="********" {...field} disabled={isPending} className="h-10 sm:h-11 lg:h-12 text-xs sm:text-sm" />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <Button type="submit" className="w-full h-10 sm:h-11 lg:h-12 text-sm sm:text-base md:text-lg font-bold" disabled={isPending}>
