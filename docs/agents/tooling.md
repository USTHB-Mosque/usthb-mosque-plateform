...
| `.playwright/`                               | Playwright config with zero tests; Playwright dependency removed per #96. **Re-introduced deliberately in #137** — see the e2e row under Kept. |
...
| `e2e/`, `playwright.config.ts`, `utils/seed-e2e.ts`              | Playwright e2e suite (canonical run in AGENTS.md commands)    | Re-introduced per #137, a deliberate reversal of #96's Playwright removal: browser-level coverage was the missing gate. Runs against a dedicated `mosque_e2e` DB + `media-e2e` bucket, never dev data. |
