...
import { previewBooksImport, commitBooksImport } from '@/features/admin/server/csv'
...
const PREVIEW_COLUMNS = ['title', 'author', 'type', 'category', 'shortDescription', 'publishDate']

const ImportBooksDialog: React.FC<ImportBooksDialogProps> = ({ open, onOpenChange }) => {
  ...
  const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    startTransition(async () => {
      try {
        const preview = await previewBooksImport(selected)
        setFile(selected)
        setResult(preview)
      } catch {
  ...
  const handleImport = () => {
    ...
    startTransition(async () => {
      try {
        ...
        if (outcome.ok) {
          toast.success(
            `تم استيراد ${outcome.created} ${outcome.created === 1 ? 'كتاب' : 'كتب'} بنجاح`,
          )
          handleClose(false)
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-3xl" showCloseButton={!pending}>
        <DialogHeader>
          <DialogTitle className="font-alyamama text-lg">استيراد الكتب من ملف CSV</DialogTitle>
        </DialogHeader>
        ...
        <div className="flex flex-col gap-5">
          ...
          {!result ? (
            ...
            >
              ...
              <span className="text-sm font-medium">اختر ملف CSV لاستيراد الكتب</span>
              <span className="max-w-md text-xs text-muted-foreground">
                الأعمدة المتوقعة: العنوان *، المؤلف *، التصنيف *، الوصف المختصر *، وصورة الغلاف
                اختيارية عبر عمود imageUrl. يتم رفض أي صف به أخطاء فيستورد الملف كلّه.
              </span>
            </button>
          ) : (
...
export default ImportBooksDialog
